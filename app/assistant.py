...
class Assistant:
    ...
    def research(self, query: str, agents: Optional[List[str]] = None):
        """Run end-to-end research workflow and return context."""
        logger.info("Assistant.research", query=query, agents=agents)
        if agents:
            return self._orchestrator.run_research_selective(query, agents)
        return self._orchestrator.run_research(query)
